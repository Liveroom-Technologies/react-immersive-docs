...
export default function App() {
  ...
  const {
    ...
    setObjectBindings: setBindings,
    updateObjectBindings,
    setBaseColor,
    clearTexture,
    copyMaterial,
    selectObjectBindings,
  } = useObjectBindings(() => structuredClone(initialBindings));
  ...
  const appliedMaterial = selectObjectBindings({ group: "shirt" })[0]?.style
    ?.material;
  ...
  // commit to whichever panel was selected. The public copyMaterial and group
  // selector helpers broadcast that panel's result across the whole shirt.
  const handleBindingsChange = useCallback(
    (next: typeof bindings) => {
      const changedEntry = Object.entries(next).find(([key, binding]) => {
        if (binding.group !== "shirt") return false;
        const previous = bindings[key];
        return (
          JSON.stringify(binding.style?.material) !==
            JSON.stringify(previous?.style?.material) ||
          binding.visible !== previous?.visible
        );
      });
      ...
      const [changedKey, source] = changedEntry;
      copyMaterial(changedKey, { group: "shirt" });
      updateObjectBindings(
        { group: "shirt" },
        { visible: source.visible },
      );
    },
    [bindings, copyMaterial, setBindings, updateObjectBindings],
  );

  const setShirtColor = useCallback(
    (hex: string) => setBaseColor({ group: "shirt" }, hex),
    [setBaseColor],
  );

  const removePrint = useCallback(() => {
    clearTexture({ group: "shirt" }, "baseColor");
