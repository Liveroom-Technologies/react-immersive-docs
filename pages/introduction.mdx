...
`@liveroom-tech/react-immersive` is a React-based 3D model viewer for
interactive GLB, GLTF, OBJ, FBX, and USDZ assets. It renders a model with React
Three Fiber, lets users click named meshes, shows a built-in side panel for the
selected object, and supports per-object actions such as color changes, texture
uploads, and visibility toggles.

## Why This Library Exists

Rendering a model in Three.js/React Three Fiber is the easy part. The hard part
is everything layered on top once a GLB, GLTF, OBJ, FBX, or USDZ asset needs to
be **interactive and configurable**:
...
- **Carries live render state** (`visible`, local `transform.position` / `transform.rotation`, plus `style.material.*` overrides like `baseColor`, `texture.path`, `opacity`, `metalness`, `roughness`, `emissive`, and the full PBR/texture-map set) so the viewer can apply overrides without you touching Three.js objects directly
...
single exhibit. The [`Guided Tour`](https://react-immersive.liveroom.dev/examples/guided-tour) demo uses exactly
this pattern.
...
- **Asset inspection tools** - [`SimpleModelViewer`](/reference/simple-model-viewer) for quickly browsing a model hierarchy without any binding setup, including an optional local upload mode for ad-hoc inspection
...
2. Loads the model with the format-appropriate loader
3. Builds stable node identifiers, including suffixed identifiers for duplicate names, and resolves `objectBindings` through those identifiers
...
- A full-height viewer layout styled by the package's explicit static CSS import
...
To use this library successfully, your model should follow these conventions:

- Binding identifiers must match the model node IDs (`BindingBuilder` assigns stable suffixed IDs when authored names are duplicated)
- Target nodes should have geometry and a `MeshStandardMaterial`
- The file must be accessible from the browser at the path passed to `modelUrl`; hosted GLTF, OBJ, and FBX files must also serve external dependencies at their declared relative paths
